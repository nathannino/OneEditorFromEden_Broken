...
if (directory_exists(FolderLocation)) {
	...
	room_restart();
	show_debug_message("Folder not found, restarting room");
	...
	show_debug_message("Debug mod active and location empty, activating shortcut");
	...
	if (directory_exists(FolderLocation)) { //Check again just to be sure
		...
		show_debug_message("Shortcut not found, restarting room");
		room_restart();
	}
